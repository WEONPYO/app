// ==========================================
// Tennis Scoreboard - Logic & Web Speech TTS
// ==========================================
...
function setupEvents() {
  ...
  const btnFsStart = document.querySelector('.btn-fullscreen-start');
  if (btnFsStart) btnFsStart.onclick = startFullscreenApp;

  const btnFsToggle = document.getElementById('btn-fullscreen-toggle');
  if (btnFsToggle) btnFsToggle.onclick = toggleFullscreen;

  const btnScoreA = document.querySelector('#card-team-a .btn-score-up');
  if (btnScoreA) btnScoreA.onclick = () => addPoint('A');

  const btnScoreB = document.querySelector('#card-team-b .btn-score-up');
  if (btnScoreB) btnScoreB.onclick = () => addPoint('B');

  const btnToggleServe = document.getElementById('btn-toggle-serve');
  if (btnToggleServe) btnToggleServe.onclick = toggleServeTeam;

  const setTypeSelect = document.getElementById('set-type');
  if (setTypeSelect) setTypeSelect.onchange = togglePlayerInputs;

  const btnCorrection = document.querySelector('.controls button:nth-child(1)');
  if (btnCorrection) btnCorrection.onclick = undo;

  const btnChangeCourt = document.querySelector('.controls button:nth-child(3)');
  if (btnChangeCourt) btnChangeCourt.onclick = manualCourtChange;

  const btnReset = document.querySelector('.controls button:nth-child(4)');
  if (btnReset) btnReset.onclick = confirmResetMatch;
}
...
function playWakeUpSound(durationMs = 200) {
